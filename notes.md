...
| **Тип Мандбельброта**         | **Среднее значение** | **Девиация**   | **Минимум**  | **Тиков на пиксель** |
| ----------------------------- | -------------------- | -------------- | ------------ | -------------------- |
...
| Тип Мандельброта  |    Среднее     |   Девиация     |    Минимум     | Тиков на пиксель  |
|-------------------|----------------|----------------|----------------|-------------------|
...
Учтем, также что O3 вероятно попытается убрать все расчеты переменной, которая хранит количество тиков для каждой из точек, так что необходимо пометить эту переменную volatile, чтобы расчеты все-таки остались.
...
| Тип Мандельброта  |    Среднее     |   Девиация     |    Минимум     | Тиокв на пиксель |
|-------------------|----------------|----------------|----------------|------------------|
...
Как видим, большой разницы нет, можем сравнить cреднее количество тиков на пиксель:
|   С графикой     |   Без графики   |
|------------------|-----------------|
|           726.63 |          725.47 |
|           264.94 |          268.97 |
|          1454.02 |         1452.38 |
...
Алгоритм с графикой не проигрывает, значит основную роль играет именно подсчет точек. Интересно, что в случаях с O3, алгоритм без графики работает дольше, что настораживает. Но, рассмотрим допустим ассемблерный код, сгенерированный для самого оптимизированного случая с O3:
```x86asm
.L14:
        vmovaps ymm15, YMMWORD PTR [rsp-32]
        vandps  ymm0, ymm0, ymm8
        vaddps  ymm1, ymm1, ymm1
        vsubps  ymm2, ymm2, ymm4
        vaddps  ymm0, ymm15, ymm0
        vfmadd132ps     ymm3, ymm6, ymm1
...
.L3:
        ...
        vcmpps  ymm0, ymm0, ymm7, 17
        vmovmskps       edx, ymm0
        test    edx, edx
        jne     .L14
```

В нем два обращения к памяти для поддержания cnt, вместо одного которое было в варианте с графикой:
```x86asm
...
        jmp     .L4
.L14:
...
А поскольку цикл внутренний и выполняется явно большее остальных количество раз и все остальные операции в нем почти невесомые, то наблюдаем такое ухудшение, используя volatile. Тогда несколько изменим код:
```c++
...
volatile __m256 save_cnt = cnt;
```

И уже получаем ассемблерный код без обращений к памяти, во внутреннем цикле:
```x86asm
.L14:
        vaddps  ymm1, ymm1, ymm1
        vsubps  ymm2, ymm2, ymm4
        vandps  ymm0, ymm0, ymm9
        vaddps  ymm5, ymm5, ymm0
        vfmadd132ps     ymm3, ymm7, ymm1
...
.L4:
        ...
        vmulps  ymm4, ymm3, ymm3
        vaddps  ymm0, ymm2, ymm4
        vcmpps  ymm0, ymm0, ymm8, 17
        vmovmskps       edx, ymm0
        test    edx, edx
        jne     .L14
```

При перетестировании самого оптимизированного кода, теперь наконец-то получаем 326.11 тиков на пиксель без O3 и 37.54 тиков на пиксель с ним. Запустим также на оптимизированном варианте без intrinsic и получим 1445.22 и 172.71.

<img align="right" width="48%" src="images/draw_opt.bmp">
<img align="left" width="30%" src="images/draw_avx.bmp", height="415">
